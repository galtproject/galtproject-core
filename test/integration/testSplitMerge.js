...
contract('SplitMerge', ([coreTeam, alice]) => {
  ...
  beforeEach(async function() {
    ...
    this.splitMerge = await SplitMerge.new();
    ...
    await this.splitMerge.initialize(this.ggr.address);
    ...
    await this.acl.setRole(bytes32('GEO_DATA_MANAGER'), this.splitMerge.address, true, { from: coreTeam });
    await this.acl.setRole(bytes32('SPACE_MINTER'), this.splitMerge.address, true, { from: coreTeam });
    await this.acl.setRole(bytes32('SPACE_BURNER'), this.splitMerge.address, true, { from: coreTeam });
  ...
  describe('package', () => {
    ...
    it('should split and then merge correctly', async function() {
      ...
      await this.splitMerge.mergeSpaceToken(clippingSpaceTokensIds[0], subjectSpaceTokenId, subjectContourGeohash5, {
        from: alice
      });
