...
library SegmentUtils {
  ...
  function compareSegments(Sweepline storage sweepline, int[2][2] a, int[2][2] b) public view returns(int8) {
    if (isEqual(a, b)) {
      return int8(0);
    }
  ...
  function getY(int[2][2] segment, int x) public pure returns (int) {
    if (x <= segment[0][0]) {
      return segment[0][1];
    ...
    if (deltaX0 > deltaX1) {
      ifac = deltaX0 / (segment[1][0] - segment[0][0]);
      fac = 1 - ifac;
    } else {
      fac = deltaX1 / (segment[1][0] - segment[0][0]);
      ifac = 1 - fac;
    }

    return (segment[0][1] * fac) + (segment[1][1] * ifac);
  }
}
