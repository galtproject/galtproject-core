...
contract CheckpointableId {
  ...
  mapping(uint256 => Checkpoint[]) internal _cachedBalances;
  ...
  function _balanceOfAt(uint256 _id, uint256 _blockNumber) internal view returns (uint256) {
    // These next few lines are used when the balance of the token is
    //  requested before a check point was ever created for this token, it
    //  requires that the `parentToken.balanceOfAt` be queried at the
    //  genesis block for that token as this contains initial balance of
    //  this token
    if ((_cachedBalances[_id].length == 0) || (_cachedBalances[_id][0].fromBlock > _blockNumber)) {
      // Has no parent
      return 0;
      // This will return the expected balance during normal situations
    } else {
      return _getValueAt(_cachedBalances[_id], _blockNumber);
    }
  }
